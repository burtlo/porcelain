...
- **Config:** `config/gateway.yaml`, `config/chimera-broker.config.json`, `provider-free-tier.yaml` (archives ship examples as starting points). Routing policy lives on assistants in operator SQLite, not a global YAML file. `config/api-keys.yaml` from setup or copy of `api-keys.example.yaml`. See [configuration.md](configuration.md).
